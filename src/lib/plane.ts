 ...
 */
export function distanceFromPointToPlane(
    pt: math.vec.Vector3 | Serie,
    plane: Plane,
): number | Serie {
    if (Serie.isSerie(pt)) {
        const S = pt as Serie
 ...
 * @param plane The plane defined with a point and its normal
 */
export function vectorFromPointsToPlane(
    pt: math.vec.Vector3 | Serie,
    plane: Plane,
...
function _distanceFromPointToPlane_(p: math.vec.Vector3, plane: Plane): number {
    ...
    const sb = sn / sd
    const B = math.vec.add(
        p,
        math.vec.scale(plane.normal, sb),
    ) as math.vec.Vector3
    return math.vec.norm(vector(p, B))
}

function _vectorFromPointToPlane_(
    p: math.vec.Vector3,
    plane: Plane,
): math.vec.Vector3 {
    const sn = -math.vec.dot(plane.normal, vector(plane.point, p, true))
