...
export class InterpolateSerieOnSurface {
     ...
     */
    interpolate(
        {serie, atTriangles = false, localCsys = true}:
        {serie: Serie, atTriangles?: boolean, localCsys: boolean}
    ): Serie{
        ...
        if (atTriangles == true) { // no interpolation
            ...
            const values : vec.Vector3[] = new Array(this.surface.nbNodes).fill([0,0,0])
