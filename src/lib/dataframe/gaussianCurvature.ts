...
export class CurvatureDecomposer implements Decomposer {
    ...
    constructor({positions, indices, names=undefined}:{positions: Serie, indices: Serie, names?: string[]}) {
        if (positions && indices) {
            this.geo = new Geometry(positions, indices, true)
        }
        if (names !== undefined) {
            if (names.length !== 4) throw new Error('curvature names length must be 4 (principal 1, principal 2, mean, gaussian)')
            this.names_ = names
        }
    }
    ...
    serie(df: DataFrame, itemSize: number, name: string): Serie {
        ...
        switch (name) {
            case this.names_[0]: return this.geo.k1() as Serie
            case this.names_[1]: return this.geo.k2() as Serie
            case this.names_[2]: return this.geo.H() as Serie
            case this.names_[3]: return this.geo.K() as Serie
...
class Geometry {
     ...
     */
    K(): Serie {
        return Serie.create({array: this.mesh.vertices.map( v => this.angleDefect(v) ), itemSize: 1})
	}
     ...
     */
    H(): Serie {
        return Serie.create({array: this.mesh.vertices.map( v => this.scalarMeanCurvature(v) ), itemSize: 1})
	}
