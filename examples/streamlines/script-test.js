...
class Normalizer {
    constructor(bounds, scaling = 1) {
        if (bounds.length !== 6) throw new Error('bounds is an array of length 6 ([xmin,ymin,zmin, xmax,ymax,zmax])')
        this.bounds = bounds
...
const grid = geom.generateRectangle({a:10, b:20, na:20, nb:20, center: [0,0,0]})
...
console.log(lines)
fs.writeFileSync('/Users/fmaerten/data/streamlines/test.pl', io.encodeGocadPL(lines), 'utf8')
fs.writeFileSync('/Users/fmaerten/data/streamlines/test.ts', io.encodeGocadTS(grid), 'utf8')
